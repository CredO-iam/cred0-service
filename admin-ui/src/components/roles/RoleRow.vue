...
<template>
  ...
  >
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div>
        <p class="font-medium">{{ props.role.name }}</p>
        <p class="text-sm opacity-80">description: {{ props.role.description || '-' }}</p>
        <p class="text-sm opacity-80">users: {{ toCsv(props.role.userIds) || '-' }}</p>
        <p class="text-sm opacity-80">groups: {{ toCsv(props.role.groupIds) || '-' }}</p>
      </div>
